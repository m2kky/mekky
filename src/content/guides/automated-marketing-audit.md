...
| وجه المقارنة | الطريقة التقليدية (يدوية) | نظام الـ Vibe Coding المدمج (الآلي) |
|:---:|:---:|:---:|
| ⏱️ **الوقت المستغرق** | 14 - 30 يوماً | دقائق معدودة |
| 🧮 **الأداة المستخدمة** | شيتات الإكسيل المعقدة (VLOOKUP, Pivot) | كود Python + AI |
| 📊 **دقة الـ CPA** | يعتمد على أرقام مدير الإعلانات (غالباً مضللة) | مطابقة بـ Shopify Source of Truth |
| 💬 **تحليل المحادثات** | قراءة يدوية أو تخمينات | تحليل NLP لمئات الرسائل لاكتشاف النمط |
| 📉 **اكتشاف المشاكل** | بعد فوات الأوان (نهاية الشهر) | فوري ومربوط مباشرة بالـ APIs |
...
> 🖼️ **تلميح مرئي:** في نافذة تثبيت بايثون، ستجد في أسفل الشاشة خياراً صغيراً ولكنه مصيري: `[ ] Add Python 3.xx to PATH`، قم بتحديده ثم انقر على `Install Now`.
...
> 🖼️ **تلميح مرئي:** عند فتح Antigravity، ستجد على اليسار القائمة الجانبية (Explorer) لفتح مجلداتك، وعلى اليمين أو في شريط جانبي آخر توجد "نافذة الدردشة" للحديث مع الذكاء الاصطناعي وكتابة الـ Prompts.
...
> `ads_read`, `read_insights`, `business_management`, `pages_show_list`, `pages_read_engagement`, `pages_manage_metadata`, `pages_messaging`, `instagram_basic`, `instagram_manage_insights`, `instagram_manage_messages`, `leads_retrieval`
...
> 🖼️ **تلميح مرئي:** في أداة `Debug Tool`، انزل إلى قسم `Extend Access Token` واضغط على الزر الأزرق ليظهر لك المفتاح الطويل الذي تحتاجه.
...
> 🖼️ **تلميح مرئي:** داخل بوابة المطورين (TikTok Developer Portal)، ستجد الـ Access Token الطويل ظاهراً في إعدادات تصاريح تطبيقك مباشرة دون الحاجة لخطوات تمديد إضافية كميتا.
...
> 🔑 **استخراج مفاتيح Google Ads:** عملية مكونة من جزأين؛ الأول من **Google Ads Manager** (Developer Token)، والثاني من **Google Cloud Console** (Client ID / Secret و OAuth).
...
> 🖼️ **تلميح مرئي:** في شوبيفاي، من قسم المبيعات `Analytics > Reports > Sales over time`، اضغط على زر التصدير `Export` في الزاوية العلوية اليمنى، وحدد `Export to CSV`.
...
> 🖼️ **تلميح مرئي:** محرر `ShopifyQL Notebook` يبدو كشاشة برمجة سوداء؛ ستكتب استعلاماتك وتشاهد الداتا تعود كجداول فورية في الأسفل.
...
> 📊 **النتيجة المتوقعة:** جدول يقسم إجمالي المبيعات حسب المصدر، وستجد غالباً اختلافا صارخاً بين المبيعات المنسوبة لـ `facebook` وبين المبيعات المكتوبة كـ `direct` أو مصادر أخرى.
...
> 🖼️ **تلميح مرئي:** الاستكمال التلقائي (Autocomplete) سيقترح عليك المقاييس `metrics` (كإجمالي المبيعات، ومعدل التحويل) والأبعاد `dimensions` (كالمناطق الجغرافية ومصادر الزيارات) بمجرد الكتابة.
...
> 🖼️ **تلميح مرئي:** في واجهة إعداد الإعلان بـ Facebook Ads Manager، قم بالنزول لأسفل خيارات الإعلان حتى تصل لحقل `URL Parameters` والصق الكود مباشرة ليتم إرفاقه مع الرابط.
...
> 📊 **النتيجة المتوقعة:** تقرير مالي مرئي يعرض كل مصدر زيارة (Referrer) وما جناه من عدد الجلسات `Sessions`، الطلبات `Orders`، وإجمالي المبيعات والمبالغ الفعلية، كل ذلك بلا برمجة.
...
**الهيكل النهائي للمجلدات لديك:**
```text
📂 ProjectX_Growth_Audit/
├── 📁 shopify/    (ضع هنا ملفات CSV من Shopify)
├── 📁 meta/       (المخرجات ستذهب هنا)
├── 📁 tiktok/     (المخرجات ستذهب هنا)
├── 📁 google/     (المخرجات ستذهب هنا)
└── 📄 .env        (ملف الأسرار سيتم إنشاؤه في الخطوة التالية)
```
...
> 🖼️ **تلميح مرئي:** يجب أن يبدو محتوى ملف الأسرار لديك، المنتهي بـ `.env`، مطابقاً للكتلة البرمجية أعلاه وبدون أي مسافات قبل أو بعد علامة يساوي `=`.
...
> 💻 **المشهد:** سيقوم البرنامج ذاتياً بكتابة الأكواد، وستجد نافذة جديدة في الأسفل (Terminal) ترسل أوامر التثبيت، وتتبادل الاتصال مع سيرفرات المنصات بشكل حي وفوري أمام ناظريك! 
...
**مثال مُبسّط من مخرجات جدول نقطة التعادل (Break-Even):**
| المنتج / الخدمة | أقصى تكلفة استحواذ (Max CPA) | العائد المستهدف الفعلي (ROAS Target) | نسبة المرتجع التقريبية |
|:---:|:---:|:---:|:---:|
| منتج A | 120 ج.م | 4.2x | 12% |
| منتج B | 300 ج.م | 3.5x | 8% |
...
> ✅ **العلامات الخضراء المطمئنة في الأداة المساعدة (Meta Pixel Helper):**
> 🟢 PageView (عند الدخول)
> 🟢 ViewContent (في صفحة العرض)
> 🟢 AddToCart (عند الإضافة للسلة)
> 🟢 InitiateCheckout (الدفع)
> 🟢 **Purchase** (الأهم، عند إتمام العملية)
...
> 💡 **الحقيقة عبر البيانات المُقاطعة:** الجدول الأعلى يُلخص قوة مقاطعة البيانات عبر طبقات البرنامج السحرية، التي تكشف التباين المرعب بين ما تظهره مديرات الإعلانات وبين الأثر المالي الذي يدخل محفظتك في العالم الواقعي.
